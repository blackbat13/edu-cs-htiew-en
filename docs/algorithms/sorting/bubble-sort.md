...
[Sortowanie bąbelkowe](https://blackbat13.github.io/visul2/sorting/bubble_sort/#array=%5B6%2C5%2C3%2C1%2C8%2C7%2C2%2C4%5D){ .md-button }
...
procedura SortowanieBabelkowe(n, A):
    1. posortowane := FALSE
    2. i := 0
    3. Dopóki posortowane = FALSE, wykonuj:
        4. posortowane := TRUE
...
%%{init: {"flowchart": {"curve": "linear"}, "theme": "neutral"} }%%
flowchart TD
    START(["SortowanieBąbelkowe(n, A)"]) --> K1[posortowane := FALSE
    i := 0]
    K1 --> K3{posortowane = FALSE}
    K3 -- TRUE --> K4[posortowane := TRUE
    j := 1]
    K4 --> K5{j <= n - i}
